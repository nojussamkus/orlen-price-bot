...
def smart_find_numbers(segment: str):
    """
    Iš segmentų (pvz. '556.31 519.60 1075.91 1225.94 1301.85')
    ištraukia realius skaičius, išvengiant sujungimų kaip '556.31519.60'
    """
    # įdėk tarpą tarp skaičių, jei nėra (pvz. 556.31519.60 -> 556.31 519.60)
...
def pick_value_for_terminal(pdf_text: str):
    ...
    # Surenkam kelių eilučių langą, nes pdfplumber kartais padalina stulpelius
    window = " ".join(lines[idx: idx + 4])
    nums = smart_find_numbers(window)

    if len(nums) < 3:
        raise RuntimeError(f"Per mažai skaičių eilutėje: {nums}")

    value = round(nums[2], 2)  # 3-ias = Bazinė kaina su akcizu
